...
fun BootScreen(onFinished: () -> Unit) {
    ...
    fun scanContainers() = scope.launch(Dispatchers.IO) {
        containers = ProcessBuilder("su", "-c", "ls -d /data/als/dev/*/ 2>/dev/null | xargs -n1 basename").start().inputStream.bufferedReader().readLines()
        ...
        uri?.let {
            scope.launch(Dispatchers.IO) {
                ...
                val fileName = context.contentResolver.query(it, null, null, null, null)?.use { cursor ->
                    if (cursor.moveToFirst()) cursor.getString(cursor.getColumnIndexOrThrow(OpenableColumns.DISPLAY_NAME)) else "rootfs"
                } ?: "rootfs"
                val tempFile = File(context.cacheDir, fileName).apply { context.contentResolver.openInputStream(it)?.use { input -> outputStream().use { out -> input.copyTo(out) } } }
                val nextIndex = (ProcessBuilder("su", "-c", "ls -d /data/als/dev/*/ 2>/dev/null | wc -l").start().inputStream.bufferedReader().readText().trim().toIntOrNull() ?: 0) + 1
                val targetDir = "/data/als/dev/$nextIndex-$fileName"
                ProcessBuilder("su", "-c", "cd /data/als && ./busybox mkdir -p $targetDir && ./busybox tar -xapf ${tempFile.absolutePath} -C $targetDir").start().waitFor()
    ...
    LaunchedEffect(Unit) {
        withContext(Dispatchers.IO) {
            if (ProcessBuilder("su", "-c", "[ ! -f /data/als/busybox ] && echo 1").start().inputStream.bufferedReader().readText().contains("1")) {
                File(context.cacheDir, "busybox").apply {
                    context.assets.open("busybox").use { i -> outputStream().use { o -> i.copyTo(o) } }
                    ProcessBuilder("su", "-c", "mkdir -p /data/als && cp $absolutePath /data/als/busybox && chmod 755 /data/als/busybox").start().waitFor()
                    delete()
                }
            }
            systemInfo = ProcessBuilder("su", "-c", "echo \"$(uname -m)\n$(/system/bin/getenforce)\n$(df /data | awk 'NR==2 {printf \"%.2f GB\", $4/1024/1024}') Free\n$(cat /sys/class/power_supply/battery/capacity)% [$(cat /sys/class/power_supply/battery/status)]\n$(uname -r)\"").start().inputStream.bufferedReader().readText()
            scanContainers()
        }
    }
    Splash(
        modifier = Modifier.pointerInput(subMenu, isRunning, finished, containerSelected) {
